...
#CHECK THESE, ARBITRARY CHOSEN AS UNIQUE
def test_create_boxer_duplicate(mock_cursor):
    """Test creating a boxer with a duplicate name, weight, height, reach, and age (should raise an error).
    ...
    with pytest.raises(ValueError, match="Boxer with name 'Boxer Name', weight 126 lb, height 77 in, reach 14.7 in, and age 18 years already exists."):
        create_boxer(name="Boxer Name", weight=126, height=77, reach=14.7, age=18)
...
def test_create_boxer_invalid_height():
    ...
    with pytest.raises(ValueError, match=r"Invalid height: invalid \(must be greater than 0\)."):
        create_boxer(name="Boxer Name", weight=126, height="invalid", reach=14.7, age="invalid")
...
def test_create_boxer_invalid_reach():
    ...
    with pytest.raises(ValueError, match=r"Invalid reach: invalid \(must be greater than 0\)."):
        create_boxer(name="Boxer Name", weight=126, height=77, reach="invalid", age="invalid")
